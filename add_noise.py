...
def add_rician_noise(image, noise_level=0.1):
...
# image = imageio.imread('s-data/IXI-DTI/IXI002-Guys-0828-DTI-00/slice_0.png', mode='F')
# 批量读取"./s-data/"下所有文件夹中的所有图片
# 存入"./n-data25/"下对应的文件夹中
for folder in os.listdir('./s-data/'):
    for mri in os.listdir('./s-data/' + folder):
        for slice in os.listdir('./s-data/' + folder + '/' + mri):
            ...
            if not os.path.exists('./n-data25/' + folder + '/' + mri):
                os.makedirs('./n-data25/' + folder + '/' + mri)
                
            imageio.imwrite('./n-data25/' + folder + '/' + mri + '/' + slice, rician_noisy_image)
            # imageio.imwrite('./n-data25/' + folder + '/' + mri + '/' + slice, gaussian_noisy_image)
